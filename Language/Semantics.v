    ...
    | FunctionCall f args =>
      ...
      | "add" =>
        ...
        | [x; y] =>
          ...
          | _ => Error (debug_print mem env e ++
            "Operation only on integers" ++ nl ++
            "Not the case for " ++ x ++ " and " ++ y)
          end
        | _ => Error "Add should have two arguments"
        end
      | "sub" =>
        match args with
        | [x; y] =>
      ...
      | _ =>
        ...
        let++ e = get_function_expression f p error (debug_print mem env e);
        semantics_expression_exec n p (mem, (env, Expression e)::exe_stack)
      end
...
Definition executeTest (code : string) : result value :=
  ...
  let+ main_expression = get_function_expression "main" p;
...
Compute executeTest "fn main(){let a = 4; a = 4; let y = &a; a}". (*OK*)
